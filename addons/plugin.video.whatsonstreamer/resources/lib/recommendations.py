...
def _finalize(simkl, scores: dict, kind: str):
    """Ranks a {simkl_id: {count, because, title, year, poster}} bucket, takes the top
    MAX_ITEMS, and backfills tmdb/imdb ids + overview + rating via one more (cached) details
    fetch per winner - needed so the result can link into the existing show/movie detail
    screens like every other WhatsUpNext entry does."""
    ranked = sorted(
        scores.items(),
...
def build(simkl) -> dict:
    ...
    shows_out = _finalize(simkl, show_scores, kind="show")
    movies_out = _finalize(simkl, movie_scores, kind="movie")
